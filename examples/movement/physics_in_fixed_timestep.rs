...
fn main() {
    App::new()
        ...
        // ! advance_physics 对于移动量进行了计算,但并没有更新 Transform 的位置
        ...
        .add_systems(
            ...
            (
                ...
                // ! 获得用户输入写在 BeforeFixedMainLoop 里面
                // ! 写在 Update 中太晚了.
                // ! 写在 FixedUpdate 中有可能不会注册玩家输入, 因为这个调度有时候每帧都不会运行.
                ...
                // ! 真正完成视觉渲染的 system,也可以写在 Update 中,
                // ! 如果写在 Update 中,使用的时间就是 RealTime, 而不是 FixedTime
                // ! system 中并没有直接对 transform 进行操作,而是对 PhysicalTranslation 进行操作
                // @ let alpha = fixed_time.overstep_fraction();                   获得插值因子
                // @ let rendered_translation = previous.lerp(current, alpha);     上次位置和当前位置进行插值
                // @ transform.translation = rendered_translation;                 完成插值
